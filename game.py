...
class ChessGame:
    ...
    def select_move(self, depth, turn):
        logging.debug("selectmove")
        bestMove = chess.Move.null()
        bestValue = -float('inf')

        for move in self.board.legal_moves:
            self.make_move(move)
            boardValue = self.__minmax(
                self.board, depth-1, -float('inf'), float('inf'), not turn)
            self.unmake_move(self.board)
            if boardValue >= bestValue:
                bestValue = boardValue
                bestMove = move
        logging.debug(self.board)
        logging.debug(bestValue)
        return bestMove.uci()
    ...
    def get_move(self):
        ...
        logging.debug(move + " has been played")
        logging.debug(self.board)
        return move

    def accept_draw(self):
        # TODO
        logging.debug('TODO check if draw should be accepted.')
        return False
    ...
    def __init_evaluate_board(self, board):
        ...
        self.boardvalue = material + pawnsq + knightsq + \
            bishopsq + rooksq + queensq + kingsq
    ...
    def __update_eval(self, move, side):
        ...
        if move.promotion != None:
            if side:
                self.boardvalue = self.boardvalue + \
                    self.piecevalues[move.promotion-1] - \
                    self.piecevalues[movingpiece-1]
    ...
    def __minmax(self, board, depth, alpha, beta, turn):
        ...
        if turn:
            ...
            for move in board.legal_moves:
                self.make_move(move)
                best_move = max(best_move, self.__minmax(
                    board, depth-1, alpha, beta, not turn))
            ...
            for move in board.legal_moves:
                self.make_move(move)
                best_move = min(best_move, self.__minmax(
                    board, depth-1, alpha, beta, not turn))
                self.unmake_move(board)
